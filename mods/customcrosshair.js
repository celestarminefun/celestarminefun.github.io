...
const crosshairPresets = {
    ...
    let container = document.querySelector(".mf-keystrokes") || document.body;
    ...
    const customWrapper = uiWrapper.querySelector("#mf-crosshair-custom-url-wrapper");
    const customInput = uiWrapper.querySelector("#mf-crosshair-custom-url");
    const sizeSlider = uiWrapper.querySelector("#mf-crosshair-size");
    ...
    customInput.addEventListener("change", ()=>{ settings.customURL = customInput.value.trim(); applyCrosshair(); saveSettings(); });
    sizeSlider.addEventListener("input", ()=>{ settings.size = parseFloat(sizeSlider.value); applyCrosshair(); saveSettings(); });
    ...
    window.addEventListener("keydown", e=>{
        ...
        if(e.code==="F8"){
            menuOpen = !menuOpen;
            uiWrapper.querySelector(".mf-crosshair-settings").style.display = menuOpen?"flex":"none";
            e.preventDefault();
        }
    }, { capture:true });
